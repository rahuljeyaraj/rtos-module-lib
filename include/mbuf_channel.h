...
class MBufChannel : public BaseChannel {
   public:
    ...
    MBufChannel(uint8_t *storage, size_t sizeBytes, StaticMessageBuffer_t *control,
                bool threadSafeWrite = false, bool threadSafeRead = false)
    ...
    ~MBufChannel() override {
        if (ownBuf_ && mbuf_) vMessageBufferDelete(mbuf_);
        if (mtxWrite_) vSemaphoreDelete(mtxWrite_);
        if (mtxRead_) vSemaphoreDelete(mtxRead_);
    }
    ...
    bool pushFromISR(const void *buf, size_t len, BaseType_t *pTaskWoken = nullptr) override {
        ...
        if (len < minMsgSizeBytes_ || len > maxMsgSizeBytes_) return false;
   ...
   private:
    void setupMutexes(bool threadSafeWrite, bool threadSafeRead) {
        if (threadSafeWrite) mtxWrite_ = xSemaphoreCreateMutex();
        if (threadSafeRead) mtxRead_ = xSemaphoreCreateMutex();
    }
    ...
    struct LockGuard {
        ...
        ~LockGuard() {
            if (m && taken == pdTRUE) xSemaphoreGive(m);
        }
        bool acquired() const {
            return taken == pdTRUE;
        }
